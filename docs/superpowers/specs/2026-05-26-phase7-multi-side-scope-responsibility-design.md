...
> **废弃说明（2026-05-27）**：本设计未进入实现，且已由新的“阶段七：深度 PRD 生成与双输入来源”取代。项目不再计划以阶段七引入 `Code Side`、`Analysis Scope`、`Responsibility Hint` 或 `Candidate Location`；本文仅保留为历史讨论记录，不得作为实现依据。
