...
class RuntimeConfig:
    ...
    def agent_config_dict(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "gateway_agent": self.gateway_agent,
            "gateway_bin": self.gateway_bin,
            "gateway_permission_policy": self.gateway_permission_policy,
            "gateway_idle_timeout": self.gateway_idle_timeout,
        }
        return config
...
def build_runtime_config(args) -> RuntimeConfig:
    ...
    if prompt_via not in ("stdin", "arg"):
        prompt_via = "stdin"
    gateway_agent = str(_first_non_empty(getattr(args, "gateway_agent", None), default=""))
    if agent == "gateway" and not gateway_agent:
        raise ValueError("--agent gateway 需要 --gateway-agent 参数")
    gateway_permission_policy = str(_first_non_empty(getattr(args, "gateway_permission_policy", None), default="best-effort-read-only"))
    if gateway_permission_policy not in ("best-effort-read-only", "strict-read-only"):
        gateway_permission_policy = "best-effort-read-only"
    return RuntimeConfig(
        ...
        gateway_bin=str(_first_non_empty(getattr(args, "gateway_bin", None), default="")),
        gateway_permission_policy=gateway_permission_policy,
        gateway_idle_timeout=_int_value(getattr(args, "gateway_idle_timeout", None), 300),
