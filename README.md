...
3. **代码依据**：代码位置总览（合并关联与证据）、影响说明、实现完成度（含需求点统计比例）
...
- 代码位置总览将 Code Impact（代码影响）的关联位置与 Completion Assessment 的证据位置合并展示。每行分别标注影响说明和证据说明，证据说明以简要摘要加需求点引用的形式控制列宽。
- Completion Assessment（完成度）严格依据有效 Requirement Point 和 Code Evidence。
- 信心度按有确认证据的需求点比例分级：全部完成为"高"，至少有一项为"中"，无任何确认为"低"。
...
|------|------|
| `高` | 所有需求点状态为完成 |
| `中` | 存在至少一个完成或部分完成的需求点（可混合无法判断） |
| `低` | 无任何完成或部分完成的需求点，或存在无效证据 |
